...
.PHONY: checks
checks:  ## run all the linting checks of the codebase (TODO: add Fortran checks if there are any)
...
format-fortran:  ## format the Fortran files
	uv run fprettify -r . -c .fprettify.rc

.PHONY: build-fortran
build-fortran:  ## build/compile the Fortran (including the extension module)
...
.PHONY: test
test:  ## run the tests (TODO: add Fortran tests)
...
virtual-environment:  ## update virtual environment, create a new one if it doesn't already exist
	# No editable needed as we're dealing with compiled package
	...
	# (Not editable install can be a bit of a gotcha when testing etc. if you forget to reinstall first,
	# can be eased by using `uv run --reinstall-package <package-name>`)
	uv sync --no-editable --all-extras --group all-dev
	uv run --no-editable pre-commit install
